...
class FastDiscriminator:
    ...
    def evaluate_noise(self, clean_vectors, noisy_vectors):
        y_clean = np.zeros(len(clean_vectors))
        y_noisy = np.ones(len(noisy_vectors))
        X = np.vstack((clean_vectors, noisy_vectors))
        y = np.concatenate((y_clean, y_noisy))

        if len(X) < 10:  # too few to split meaningfully
            return 0.5  # assume ambiguous, skip evaluation
